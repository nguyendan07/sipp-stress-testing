...
def convert_wav_to_pcap(wav_file, pcap_file, src_ip="192.168.1.1", dst_ip="192.168.1.2", 
                         src_port=10000, dst_port=20000, payload_type=0, packet_size=160):
    """
    ...
    try:
        ...
        with wave.open(wav_file, 'rb') as wav:
            ...
            for i in range(0, len(audio_data), packet_size):
                ...
                # Create RTP packet
                rtp = RTPPacket(payload_type=payload_type, seq_num=seq_num, timestamp=timestamp)
...
def main():
    parser = argparse.ArgumentParser(description='Convert WAV files to PCAP format for SIPp')
    
    parser.add_argument('-i', '--input', required=True, 
                        help='Input WAV file or directory')
    parser.add_argument('-o', '--output', required=True,
                        help='Output PCAP file or directory')
    ...
    parser.add_argument('--src-port', type=int, default=10000,
                        help='Source UDP port (default: 10000)')
    parser.add_argument('--dst-port', type=int, default=20000,
    ...
    parser.add_argument('--packet-size', type=int, default=160,
    ...
    if os.path.isdir(args.input):
        ...
        success = convert_wav_to_pcap(
            ...
            payload_type=args.payload_type, packet_size=args.packet_size
        )
        if success:
